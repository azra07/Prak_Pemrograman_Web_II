...
<body>
                ...
                <tr>
                    ...
                    <td>
                        <input
                            ...
                            name="pinjam"
                            id="pinjam"
                            value="<?php echo isset($result[0]['tgl_pinjam']) ? htmlspecialchars($result[0]['tgl_pinjam']) : ''; ?>"
                            required
                        >
                    </td>
                </tr>
    ...
    <!-- Script Validasi Sisi Client -->
    <script>
        const pinjam = document.getElementById('pinjam');
        const kembali = document.getElementById('kembali');

        pinjam.addEventListener('change', function() {
    ...
    if (isset($_POST['submit']) || isset($_POST['update'])) {
        ...
        if (strtotime($kembali) < strtotime($pinjam)) {
            echo "<script>alert('Tanggal pengembalian tidak boleh lebih awal dari tanggal pinjam!');</script>";
        } else {
            if (isset($_POST['submit'])) {
                tambahpeminjaman($pinjam, $kembali);
            } else {
                updatepeminjaman($_POST['id_peminjaman'], $pinjam, $kembali);
            }
        }
    }
    ?>
